<script>
  ...
  function animateScroll({ viewHeight }) {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: self => {
        ...
        if (prog <= IMPACT_POINT) {
          // Phase 1: carré descend
          const startY = -viewHeight * 0.7;
          const endY   = sol.getBoundingClientRect().top - 100;
          const y      = startY + (prog / IMPACT_POINT) * (endY - startY);
          gsap.set(carre, { y });
          sol.style.opacity = 1;
          gsap.set(".fragment", { opacity: 0 });
        } else if (prog <= BREAK_POINT) {
          // Phase 2: légère enfoncement
          const p = (prog - IMPACT_POINT) / (BREAK_POINT - IMPACT_POINT);
          const y = sol.getBoundingClientRect().top - 100 + p * 20;
          gsap.set(carre, { y });
          sol.style.opacity = 1;
          gsap.set(".fragment", { opacity: 0 });
        } else {
          // Phase 3: cassure et dispersion
          const p = (prog - BREAK_POINT) / (1 - BREAK_POINT);
          const startY = sol.getBoundingClientRect().top - 80;
  ...
  onMount(() => {
    ...
    // Initial positions
    gsap.set(carre, { opacity: 1, y: -viewHeight * 0.7 });
    gsap.set(".fragment", { opacity: 0, x: 0, y: 0, rotation: 0 });

    setFragmentsContainerPosition();
    window.addEventListener('scroll', setFragmentsContainerPosition);

    animateScroll({ viewHeight });
...
<style>
  ...
  .fragments-container { position:fixed; }
  .fragment {
    position:absolute;
    left:50%; top:50%;
